function [cj resp_t interval hasconfirmed] = cjSlider_eeg(window,vars,cfg,fillCoords,numOfFlips,colourArr,trials,t,answered,useport)
...
while ~hasconfirmed
    while any(buttons)||~haschanged   % wait for release and change of cj and confirmation
        [resp_x, resp_y, buttons] = GetMouse();
            %add trigger for confidence rating
    if (vars.triggers)
        sendTrig(51,useport)
    end
